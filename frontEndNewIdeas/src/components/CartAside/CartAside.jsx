        ...
        <div>
          <div className="fixed justify-around py-2 items-center  right-0 top-0 h-screen w-[80%] sm:w-[70%] md:w-[50%] lg:w-[20%] bg-white   px-2 text-center flex flex-col gap-2 transition-all overflow-y-scroll ">
            ...
            {cart.products.map((product) => (
              <div className="flex  justify-center border border-red-200 p-2 rounded-md m-2 w-full bg-[#D3C4E3] " key={product.id}>

                <img className="max-w-[120px] rounded-full border border-red-200 " src={product.img} alt="" />

                <div className="flex flex-col  justify-center items-center gap-1 ">
                <h3 className="text-sm tracking-tighter mb-2 ">{product.name}</h3>
                <h3 className="text-sm">Stock: {product.stock}</h3>
                <h3 className="text-sm">Precio: ${product.price}</h3>
                <div className="flex items-center">
              <button onClick={() => decrementQuantity(product.id)} className="px-2 py-1 bg-red-200 rounded-md">
                -
              </button>
              <h3 className="text-sm mx-2">Cantidad: {quantities[product.id]}</h3>
              <button onClick={() => incrementQuantity(product.id)} className="px-2 py-1 bg-green-200 rounded-md">
                +
              </button>
            </div>
                </div>
                <div className="flex  ">
                <button className="flex w-full" onClick={() => handlerRemoveProducts(product)}>
                  <img className="items-start max-w-4 border border-red-200 rounded-md bg-white hover:scale-110" src={crossRed} alt="" />
                </button>
                </div>
              </div>
            ))}
